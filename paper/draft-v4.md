...
User-supplied FASTA builds use the same package-generation path after sequence download. For URL-based inputs, GitHub Actions downloads the FASTA directly from the provided HTTP(S) URL. For browser uploads, the API creates a signed upload URL, stores the FASTA in a Cloudflare R2 bucket and passes a signed download URL to the build workflow. Both uncompressed FASTA and gzip-compressed FASTA are accepted with `.fa`, `.fasta`, `.fna` or `.fas` filename extensions. Because supplied sequence data may be private or unpublished, the web interface warns users that resulting packages are temporary downloads and are not published to the shared repository.
