...
autoBSgenome was built around a simple premise: an assembly accession should be enough. For public assemblies, the service converts NCBI and Ensembl identifiers into installable BSgenome packages without local software, command-line interaction or seed-file authoring. For assemblies not yet deposited in public databases, users can upload a FASTA file and receive a temporary BSgenome package for local installation. In parallel, autoBSgenome systematically pre-builds and indexes packages from public sources, so that many users can install directly from a CRAN-like repository rather than rebuild. The goal is concrete: no BSgenome should be hard to build.

## Materials and Methods

### System overview

autoBSgenome consists of a web frontend, an API layer, a containerized build workflow and a package repository. The frontend collects either an NCBI accession, an Ensembl species/assembly identifier or a user FASTA upload. The API validates the request and triggers a GitHub Actions workflow through a repository dispatch payload. The build workflow runs in a Docker image containing R, Bioconductor, BSgenome, BSgenomeForge, faToTwoBit, the NCBI Datasets CLI and project scripts. Successful builds produce standard source tarballs installable through `install.packages(..., repos = NULL, type = "source")`.

The public repository is maintained separately from temporary on-demand builds. Packages built from public NCBI or Ensembl inputs can be added to the permanent index. Packages built from user-uploaded FASTA files are returned as temporary GitHub Release downloads and are not added to the public repository, because autoBSgenome cannot verify public provenance for user-supplied sequence data.
...
User-uploaded FASTA builds use the same package-generation path after sequence download. The API creates a signed upload URL, stores the FASTA in a Cloudflare R2 bucket and passes a signed download URL to the build workflow. Both uncompressed FASTA and gzip-compressed FASTA are accepted with `.fa`, `.fasta`, `.fna` or `.fas` filename extensions. Because uploaded sequence data may be private or unpublished, the web interface warns users that resulting packages are temporary downloads and are not published to the shared repository.
...
The same backend also supports FASTA upload for assemblies not yet represented in NCBI or Ensembl. In this mode, the user still provides package metadata but supplies sequence data directly. Uploaded FASTA builds are useful for draft assemblies, teaching examples and controlled deployments with appropriate artifact-storage policies. They are deliberately excluded from the public repository index unless the sequence source is later archived and represented by public provenance.
...
**Figure 1. autoBSgenome workflow.** Accession or FASTA input, metadata retrieval, circular-sequence detection, FASTA acquisition, 2bit conversion, seed-file generation, package forging, R CMD build and package delivery.
...
**Figure 5. Repository browse interface.** Screenshot of the package search and package detail view, showing provider, accession, organism, assembly and installation command.
